...
SWEP.WeaponName = "cotskb_tw" -- (String) Name of the weapon script
SWEP.PrintName = "cotskb tww" -- (String) Printed name on menu
...
function SWEP:Think()
	if SERVER then
		if nextheal <= CurTime() and self.Owner.ForceAnimSequence == self.Owner:LookupSequence("0_cult_ritual") then
...
function SWEP:PrimaryAttack( right )
	...
	if SERVER then
		...
		if self.Owner.ForceAnimSequence == self.Owner:LookupSequence("0_cult_ritual") then
		...
		elseif self.Owner.ForceAnimSequence == nil and self.Owner:GetMoveType() == MOVETYPE_WALK then
			...
			self.Owner:SetForcedAnimation('0_cult_ritual', 60, function()
