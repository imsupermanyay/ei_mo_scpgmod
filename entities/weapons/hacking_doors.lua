...
function SWEP:PrimaryAttack()
	...
	if ( ent:GetClass() == "func_button" ) then
		self:GetOwner():ConCommand("new_hacker_panel")
...
net.Receive("SendHack", function(len, ply)
	...
	--print(ply)
	if IsValid(ply:GetActiveWeapon()) and ply:GetActiveWeapon():GetClass() != "hacking_doors" then return end
	...
	if ( ent:GetClass() == "func_button" ) then
		...
		if ent:GetPos() == Vector(7704, -3967, 54.3) then
			time = 180
		end

		if ent:GetPos() == Vector(7784, -4160.59, 54.3) then
			time = 180
		end

		if ent:GetPos() == Vector(7048, -2095, 54.3) then
			time = 180
		end

		if ent:GetPos() == Vector(6632, -2303, 54.3) then
			time = 180
		end

		if ent:GetPos() == Vector(6226.01, -2041, 54.41) then
			time = 180
		end

		if ent:GetPos() == Vector(6171, -2320.99, 53.41) then
			time = 180
		end

		if ent:GetPos() == Vector(8264, -4451.59, 54.99) then
			time = 180
		end
	...
	self.Owner:BrProgressBar( "l:hacking_door", time, "nextoren/gui/icons/hacker_hack.png", ent, false, function()
		if math.random(1,100) > chance then
			self.Owner:EmitSound( "nextoren/others/access_denied.wav" )
		else
			self.Owner:EmitSound( "nextoren/others/chaos_radio_open.wav" )
