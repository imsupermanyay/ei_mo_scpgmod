...
function ENT:Use( activator, caller )
  ...
  if caller:GTeam() == TEAM_GUARD or caller:GTeam() == TEAM_NTF or caller:GTeam() == TEAM_QRT or caller:GTeam() == TEAM_OSN or caller:GTeam() == TEAM_ALPHA1 then
    --[[if !IsBigRound() then
    ...
    if timer.TimeLeft("RoundTime") < 300 then
      caller:RXSENDNotify("你成功呼叫了ETTRA, 但是时间太晚了...")
      return
    end
    caller:BrProgressBar("发送信息中...", 5, "nextoren/gui/icons/hand.png", target, false, function()
      ...
      BREACH.Players:ChatPrint( player.GetAll(), true, true, "[通讯] 我们接收到了请求 一支应急响应小组即将抵达设施" )
        timer.Create("FAF_Iagent", 5, 1, function()
        BREACH.PowerfulETTSupport(nil,nil,4)
...
function ENT:Draw()
  ...
  if ( self:GetHacking() ) then
    ...
    self.ObrIcon = Material( "nextoren/gengxygao/site_19_n7.png" )
